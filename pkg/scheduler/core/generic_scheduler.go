...
func addNominatedPods(pod *v1.Pod, meta predicates.PredicateMetadata,
	...
	*schedulernodeinfo.NodeInfo) {
	...
	for _, p := range nominatedPods {
		if scheduling.GetPodPriority(p) >= scheduling.GetPodPriority(pod) && p.UID != pod.UID {
...
func pickOneNodeForPreemption(nodesToVictims map[*v1.Node]*schedulerapi.Victims) *v1.Node {
	...
	for i := 0; i < lenNodes1; i++ {
		...
		highestPodPriority := scheduling.GetPodPriority(victims.Pods[0])
	...
	for i := 0; i < lenNodes2; i++ {
		...
		for _, pod := range nodesToVictims[node].Pods {
			...
			sumPriorities += int64(scheduling.GetPodPriority(pod)) + int64(math.MaxInt32+1)
		}
	...
	if len(nomNodeName) > 0 {
		if nodeInfo, found := nodeNameToInfo[nomNodeName]; found {
			...
			for _, p := range nodeInfo.Pods() {
				if p.DeletionTimestamp != nil && scheduling.GetPodPriority(p) < podPriority {
